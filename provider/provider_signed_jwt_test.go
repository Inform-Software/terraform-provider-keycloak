...
func TestAccKeycloakProvider_signedJWT(t *testing.T) {
	...
	keycloakClient, err = keycloak.NewKeycloakClient(testCtx, os.Getenv("KEYCLOAK_URL"), "", os.Getenv("KEYCLOAK_CLIENT_JWT_ID"), os.Getenv("KEYCLOAK_CLIENT_SECRET"), os.Getenv("KEYCLOAK_REALM"), "", "", "", "", "", jwtSigningKey, true, 5, "", false, userAgent, false, map[string]string{
		"foo": "bar",
	})
	if err != nil {
		panic(err)
